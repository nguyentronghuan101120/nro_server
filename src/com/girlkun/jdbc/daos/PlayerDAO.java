...
public class PlayerDAO {
    ...
    public static boolean createNewPlayer(int userId, String name, byte gender, int hair) {
        try {
            ...
            GirlkunDB.executeUpdate("insert into player"
                    ...
                    + "data_black_ball, data_side_task, data_card, bill_data, data_item_time_sieu_cap, info_phoban, data_may_do, info_achievement, data_duoi_khi) "
                    ...
                    data_card, bill_data, itemTimeSC, info_phoban, itemMayDo, info_achive, dkhi);
    ...
    // ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() -
    // player.itemTime.lastTimeBoHuyetSC))
    // : 0));
    // dataArray.add((player.itemTime.isUseBoKhiSC
    // ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() -
    // player.itemTime.lastTimeBoKhiSC))
    // : 0));
    // dataArray.add((player.itemTime.isUseGiapXenSC
    // ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() -
    // player.itemTime.lastTimeGiapXenSC))
    // : 0));
    // dataArray.add((player.itemTime.isUseCuongNoSC
    // ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() -
    // player.itemTime.lastTimeCuongNoSC))
    // : 0));
    // dataArray.add((player.itemTime.isUseAnDanhSC
    // ? (ItemTime.TIME_ITEM - (System.currentTimeMillis() -
    // player.itemTime.lastTimeAnDanhSC))
    // : 0));
    // String itemTimeSC = dataArray.toJSONString();
    // dataArray.clear();

    // String query = " update player set data_item_time_sieu_cap =?, head = ?,
    // have_tennis_space_ship = ?,"
    // + "clan_id_sv" + Manager.SERVER
    // + " = ?, data_inventory = ?, data_location = ?, data_point = ?,
    // data_magic_tree = ?,"
    // + "items_body = ?, items_bag = ?, items_box = ?, items_box_lucky_round = ?,
    // friends = ?,"
    // + "enemies = ?, data_intrinsic = ?, data_item_time = ?, data_task = ?,
    // data_mabu_egg = ?, pet = ?,"
    // + "data_black_ball = ?, data_side_task = ?, data_charm = ?, skills = ?,"
    // + " skills_shortcut = ?, pointPvp=?, NguHanhSonPoint=?,data_card=?,bill_data
    // =? where id = ?";
    // GirlkunDB.executeUpdate(query,
    // itemTimeSC,
    // player.head,
