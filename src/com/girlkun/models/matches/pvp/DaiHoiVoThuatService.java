...
public class DaiHoiVoThuatService {
    ...
    public void Update() {
        ...
        daihoi.listReg.stream().filter((me) -> (me != null && me.zone.map.mapId != 52 && me.zone.map.mapId != 51
                && DaiHoiVoThuat.gI().Minutes >= daihoi.min_start_temp)).map((me) -> {
        ...
        if (DaiHoiVoThuat.gI().Second == 0 && DaiHoiVoThuat.gI().Minutes < daihoi.min_start_temp) {
            if (daihoi.listReg.size() > 1) {
                Service.gI().sendThongBao(daihoi.listReg, "Vòng " + daihoi.round + " sẽ bắt đầu sau "
                        + (daihoi.min_start_temp - DaiHoiVoThuat.gI().Minutes) + " phút nữa");
            }
    ...
    public boolean CanReg(Player pl) {
        return daihoi != null && pl.isPl() && Util.contains(daihoi.Time, String.valueOf(DaiHoiVoThuat.gI().Hour))
                && DaiHoiVoThuat.gI().Minutes <= daihoi.min_limit && !PlayerExits((int) pl.id);
    }

    public String Giai(Player pl) {
        if (daihoi != null && PlayerExits((int) pl.id)) {
            return "Đại hội võ thuật sẽ bắt đầu sau " + (daihoi.min_start_temp - DaiHoiVoThuat.gI().Minutes)
                    + " phút nữa";
        } else if (daihoi != null && Util.contains(daihoi.Time, String.valueOf(DaiHoiVoThuat.gI().Hour))
                && DaiHoiVoThuat.gI().Minutes <= daihoi.min_limit) {
            return "Chào mừng bạn đến với đại hội võ thuật\nGiải " + daihoi.NameCup + " đang có "
                    + daihoi.listReg.size() + " người đăng ký thi đấu";
        }
