...
public class ItemService {
    ...
    public Item otpts(short tempId, int quantity) {
        ...
        if (item.template.type == 0) {
            ...
            item.itemOptions.add(new ItemOption(47, Util.nextInt(2000, 5000)));
        }
        if (item.template.type == 1) {
            item.itemOptions.add(new ItemOption(21, 80));
            item.itemOptions.add(new ItemOption(22, Util.nextInt(1500, 2000))); // hp đơn vị (k)
        }
        if (item.template.type == 2) {
            item.itemOptions.add(new ItemOption(21, 80));
            item.itemOptions.add(new ItemOption(0, Util.nextInt(20000, 30000)));
        }
        if (item.template.type == 3) {
            item.itemOptions.add(new ItemOption(21, 80));
            item.itemOptions.add(new ItemOption(23, Util.nextInt(1500, 2000))); // ki đơn vị (k)
        }
    ...
    // phần code mở Hòm tiếp tế ra set kích hoạt
