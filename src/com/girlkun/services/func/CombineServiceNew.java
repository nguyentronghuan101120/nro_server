...
public class CombineServiceNew {
        ...
        switch (player.combineNew.typeCombine) {
            ...
            case NANG_CAP_BONG_TAI_CAP4:
                if (player.combineNew.itemsCombine.size() == 2) {
                    ...
                    if (bongTai != null && mvbt != null && mvbt.quantity >= 999) {
                        ...
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                "Cần 1 Bông tai Porata cấp 3 và X999 Mảnh vỡ bông tai ", "Đóng");
                    }
                } else {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                            "Cần 1 Bông tai Porata cấp 2 và X999 Mảnh vỡ bông tai ", "Đóng");
            ...
            case MO_CHI_SO_BONG_TAI_CAP4:
                if (player.combineNew.itemsCombine.size() == 3) {
                    ...
                    if (bongTai != null && gallery != null && daXanhLam != null && gallery.quantity >= 99) {
                        ...
                        this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                                "Cần 1 Bông tai Porata cấp 4, X99 Đá Gallery và 15 Đá xanh lam", "Đóng");
                    }
                } else {
                    this.baHatMit.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                            "Cần 1 Bông tai Porata cấp 4, X99 Đá Gallery và 15 Đá xanh lam", "Đóng");
            ...
            case CHE_TAO_TRANG_BI_TS:
                ...
                if (player.combineNew.itemsCombine.size() >= 2 && player.combineNew.itemsCombine.size() < 5) {
                    ...
                    // + "|2|Đá nâng cấp " +
                    // player.combineNew.itemsCombine.stream().filter(Item::isDaNangCap).findFirst().get().typeDanangcap()
                    // + " (+" + (daNC.template.id - 1073) + "0% tỉ lệ thành công)\n"
                    // + "|2|Đá may mắn " +
                    // player.combineNew.itemsCombine.stream().filter(Item::isDaMayMan).findFirst().get().typeDaMayman()
            ...
            case PHAN_RA_DO_THAN_LINH:
                if (player.combineNew.itemsCombine.size() == 0) {
                    this.granala.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Con hãy đưa ta đồ thần linh để phân rã",
                            "Đóng");
                    return;
                }
                if (player.combineNew.itemsCombine.size() == 1) {
                    ...
                    // }
                    // if (couponAdd == 0) {
                    // this.granala.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Ta chỉ có thể
                    // chuyển ngọc bội", "Đóng");
                    // return;
                    // }
                    if (item.template.id <= 2068) {
                        this.granala.createOtherMenu(player, ConstNpc.IGNORE_MENU, "Ta chỉ có thể chuyển ngọc bội",
                                "Đóng");
                        return;
                    }
                    if (item.template.id >= 2074) {
                    ...
                    this.granala.createOtherMenu(player, ConstNpc.IGNORE_MENU,
                            "Ta chỉ có thể phân rã 1 lần 1 loại ngọc bội", "Đóng");
                }
                break;
            case NANG_CAP_DO_TS:
                if (player.combineNew.itemsCombine.size() == 0) {
    ...
    public void openCreateItemAngel(Player player) {
        ...
        if (InventoryServiceNew.gI().getCountEmptyBag(player) > 0) {// check chỗ trống hành trang
            ...
            if (Util.nextInt(0, 100) < tilemacdinh) {
                ...
                short[][] itemIds = { { 1048, 1051, 1054, 1057, 1060 }, { 1049, 1052, 1055, 1058, 1061 },
                        { 1050, 1053, 1056, 1059, 1062 } }; // thứ tự td - 0,nm - 1, xd - 2 theo id trong id_template
    ...
    private void moChiSoBongTaicap3(Player player) {
        if (player.combineNew.itemsCombine.size() == 3) {
            ...
            if (bongtai != null && gallery != null && gallery.quantity >= 99) {
                player.inventory.gold -= gold;
                player.inventory.gem -= gem;
                InventoryServiceNew.gI().subQuantityItemsBag(player, gallery, 99);
    ...
    // }
    // if (bongtai != null && thachPhu != null && thachPhu.quantity >= 99 &&
    // daxanhlam != null && daxanhlam.quantity >= 15) {
    // player.inventory.gold -= gold;
    // player.inventory.gem -= gem;
    // InventoryServiceNew.gI().subQuantityItemsBag(player, thachPhu, 99);
    // InventoryServiceNew.gI().subQuantityItemsBag(player, daxanhlam, 10);
    ...
    private void nangCapVatPham(Player player) {
        if (player.combineNew.itemsCombine.size() >= 2 && player.combineNew.itemsCombine.size() < 4) {
            if (player.combineNew.itemsCombine.stream().filter(item -> item.isNotNullItem() && item.template.type < 4)
                    .count() != 1) {
                return;
            }
    ...
    private boolean isDaPhaLe(Item item) {
        return item != null && (item.template.type == 30 || item.template.type == 32
                || (item.template.id >= 14 && item.template.id <= 20));
    }

    private boolean isTrangBiPhaLeHoa(Item item) {
        if (item != null && item.isNotNullItem()) {
            if (item.template.type < 6 || item.template.type == 11 || item.template.type == 32
                    || item.template.type == 72 || item.template.type == 23 || item.template.type == 24
