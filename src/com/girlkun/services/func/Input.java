...
public class Input {
        ...
        try {
            ...
            switch (player.iDMark.getTypeInput()) {
                ...
                case TAI:
                    ...
                    try {
                        if (tv2 != null && tv2.quantity >= sotvtai) {
                            InventoryServiceNew.gI().subQuantityItemsBag(player, tv2, sotvtai);
                ...
                case XIU:
                    ...
                    try {
                        if (tv1 != null && tv1.quantity >= sotvxiu) {
                            InventoryServiceNew.gI().subQuantityItemsBag(player, tv1, sotvxiu);
                            InventoryServiceNew.gI().sendItemBags(player);
                            int TimeSeconds = 10;
                            Service.gI().sendThongBao(player, "Chờ 10 giây để biết kết quả.");
                            while (TimeSeconds > 0) {
                ...
                case QUY_DOI_HONG_NGOC:
                    int ratioruby = 1; // tỉ lệ đổi hồng Ngọc
                    int coinruby = 1000; //
                    ...
                    } else if (player.getSession().coinBar >= rubyTrade * coinruby) {
                        PlayerDAO.subcoinBar(player, rubyTrade * coinruby);
                        player.inventory.ruby += rubyTrade * 1000;//
    ...
    public void createFormGiveItem1(Player pl) {
    ...
    public void createFormQDHN(Player pl) {
        ...
        createForm(pl, QUY_DOI_HONG_NGOC, "Quy đổi hồng ngọc"
                ...
                new SubInput("Nhập số lượng tiền muốn đổi sang hồng ngọc theo đơn vị 'nghìn VNĐ'", NUMERIC));
    }
